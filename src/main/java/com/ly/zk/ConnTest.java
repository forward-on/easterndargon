    ...
    public static void main(String[] args) throws Exception {
        Watcher watcher = new Watcher() {
            ...
            public void process(WatchedEvent event) {
                ...
                System.out.println("触发了" + event.getPath() + "事件！");
                System.out.println("触发了" + event.getState() + "事件！");
            }
        };

        // 1.连接ZooKeeper服务器
        // 第一个参数：ZooKeeper服务器的连接地址，如果ZooKeeper是集群模式或伪集群模式（即ZooKeeper服务器有多个），那么每个连接地址之间使用英文逗号间隔，
        //             单个连接地址的语法格式为"主机IP:ZooKeeper服务器端口号"；
        // 第二个参数：session超时时长（单位：毫秒）
        // 第三个参数：用于监控目录节点数据变化和子目录状态变化的Watcher对象
        ZooKeeper zooKeeper = new ZooKeeper("127.0.0.1:2181,127.0.0.1:2182,127.0.0.1:2183", 5000, watcher);
